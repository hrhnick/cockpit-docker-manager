// Docker Manager - Shared Table Management Module (Simplified and optimized)
(function() {
    'use strict';
    ...
    function handleSort(tableId, columnIndex) {
        ...
        updateSortIndicators(tableId, columnIndex, state.sortOrder);
    ...
    function updateSortIndicators(tableId, columnIndex, sortOrder) {
        const tableElement = document.getElementById(tableId);
        const indicators = tableElement.querySelectorAll('.sort-indicator');
    ...
    function showLoading(tableId) {
        ...
        if (tbody) {
            ...
            tbody.innerHTML = `<tr class="loading-row"><td colspan="${table?.config.columns.length || 1}" class="loading-cell">${utils().escapeHtml(table?.config.loadingMessage || 'Loading...')}</td></tr>`;
    ...
    function showError(tableId, message) {
        ...
        if (tbody) {
            tbody.classList.remove('loading');
            tbody.innerHTML = `<tr class="error-row"><td colspan="${table?.config.columns.length || 1}" class="error-message">${utils().escapeHtml(message || table?.config.errorMessage || 'Error')}</td></tr>`;
